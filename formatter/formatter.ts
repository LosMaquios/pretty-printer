...
export class Formatter {
  ...
  private _getChildFormatter(
    skipFirstLineIdentation?: boolean,
    escapeString = this.options.escapeString,
  ) {
  ...
  private _formatObjectEntries(
    ...
    while (!(item = contentIterator.next()).done) {
      ...
      if (!(entry instanceof Entry)) {
        const valueFormatter = this._getChildFormatter(false);

        content += `${valueFormatter.format(entry)},\n`;
      } else {
        const keyFormatter = this._getChildFormatter(false, true);
    ...
    return `${this._formatFirstLine(constructorName)} ${
      this._wrap([
        wrapper[0],
